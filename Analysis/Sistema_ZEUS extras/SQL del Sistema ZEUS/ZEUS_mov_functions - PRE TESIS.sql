...
CASE ban
	  ...
	  INSERT INTO pedido VALUES (cod,'VIG',emp,suc,current_timestamp);--SELECT * FROM PEDIDO
          raise notice '%','NOPE';
          return 0;
      
      WHEN 2 THEN
	UPDATE pedido SET est_cod='APR' WHERE ped_cod=cod; 
	raise notice '%','NOPE';--SELECT * FROM ESTADO
	return 0;
	
      WHEN 3 THEN
	  select count(1) into existe_cod from v_ordc where ped_cod=cod and est_cod='VIG';
        if existe_cod >0 then 
	  raise notice '%','No se puede Anular un Pedido de Materiales que ya posee Orden de Compra';
	  return 1;
        else 
	  UPDATE pedido SET est_cod='ANU' WHERE ped_cod=cod;
	  raise notice '%','NOPE';
	  return 0;
        end if;
      
      WHEN 4 THEN UPDATE pedido SET est_cod='O.T' WHERE ped_cod=cod;
        raise notice '%','NOPE';
        return 0;

      WHEN 5 THEN UPDATE pedido SET est_cod='O.P' WHERE ped_cod=cod; 
        raise notice '%','NOPE';
        return 0;
...
--else
		INSERT INTO detalle_ped VALUES(cod,mat,'PEN',cant,FALSE);--select * from estado
--end if;

      WHEN 4 THEN UPDATE detalle_ped SET est_cod='ANU' WHERE ped_cod=cod AND mat_cod=mat;--rechaza

      WHEN 5 THEN UPDATE detalle_ped SET est_cod='APR' WHERE ped_cod=cod AND mat_cod=mat;--aprueba

      WHEN 6 THEN UPDATE detalle_ped SET det_ord=TRUE WHERE ped_cod=cod AND mat_cod=mat;--pone bandera de "con orden"
      
      WHEN 7 THEN UPDATE detalle_ped SET det_cant=cant,est_cod='APR' WHERE ped_cod=cod AND mat_cod=mat;--aprueba y modifica cant
...
CASE ban
      WHEN 1 THEN
          ...
          INSERT INTO ord_compra VALUES (cod,'VIG',suc,ped,emp,prov,tip,current_timestamp);		
          raise notice '%','NOPE';
          return 0;
          
      WHEN 2 THEN UPDATE ord_compra SET est_cod='VIG' WHERE ord_nro=cod;
	  raise notice '%','NOPE';
          return 0;
	
      WHEN 3 THEN
	select count(1) into existe_cod from v_com where ord_nro=cod and est_cod='VIG';
        if existe_cod >0 then 
	  raise notice '%','No se puede Anular una Orden de Compra de Materiales que ya posee Compra';
	  return 1;
        else --no posee ninguna compra aun
		UPDATE ord_compra SET est_cod='ANU' WHERE ord_nro=cod;--anula
        if ped>0 then
		SELECT COUNT(1) into existe_cod FROM v_ordc WHERE ped_cod=ped and est_cod!='ANU';--select count(1) from v_ordc WHERE ped_cod=4 and est_des!='ANU';--select * from v_ped
	  
					if existe_cod>0 then
						UPDATE pedido SET est_cod='O.P' WHERE ped_cod=ped;--ord.p
					else
						UPDATE pedido SET est_cod='APR' WHERE ped_cod=ped;--apr
					end if;
        end if;
	  raise notice '%','NOPE';
	  return 0;
      end if;
      WHEN 4 THEN UPDATE ord_compra SET est_cod='C.P' WHERE ord_nro=cod;
        raise notice '%','NOPE';
        return 0;

      WHEN 5 THEN UPDATE ord_compra SET est_cod='C.T' WHERE ord_nro=cod; 
        raise notice '%','NOPE';
        return 0;
...
CASE ban
	  ...
	  INSERT INTO compra VALUES (cod,'VIG',suc,emp,ord,nro,current_timestamp);
	  ...
	  if ord>0 then--es segun una orden
			select count(1) into existe_cod from v_com where ord_nro=ord and est_cod!='ANU';
			if existe_cod>0 then --com.p
				update ord_compra set est_cod='C.P' where ord_nro=ord;
			else --vig
				update ord_compra set est_cod='VIG' where ord_nro=ord;
			end if;
		end if;
	  UPDATE compra SET est_cod='ANU' WHERE com_cod=cod;
	  raise notice '%','NOPE';
	  return 0;
       -- end if;
end case;
...
raise notice '%','Se designó el Codigo '||cod||' para evitar conflictos';
      ...
      INSERT INTO ajuste_m VALUES (cod,'VIG',emp,current_date);
      raise notice '%','NOPE';
      return 0;
      
      WHEN 3 THEN UPDATE ajuste_m SET est_cod='ANU' WHERE aju_cod=cod;
      raise notice '%','NOPE';
      return 0;
end case;
    end;
...
CREATE OR REPLACE FUNCTION public.abm_dajum(
    ban integer,
    cod integer,
    mat_ integer,
    dep integer,
...
begin
CASE ban
        WHEN 1 THEN --select * from v_dajum
    if tmot>1 then-------------------------------------es un ajuste positivo------------------------------------------------------------------------
select count(1) into find from detalle_ajuste_m where mat_cod=mat_ and aju_cod=cod and dep_cod=dep and suc_cod=suc and mot_cod=mot and tmot_cod!=tmot;--verifica si se ingreso un detalle negativo previamente (anularia el corriente detalle)
...
insert into detalle_ajuste_m values(cod,mat_,dep,suc,mot,tmot,cant_);
update stock_mat set cant=cant+cant_ where suc_cod=suc and dep_cod=dep and mat_cod=mat_;
	--raise notice '%','NOPE';
      --  return 0;
end if;

	else----------------------------------------------es un ajuste negativo---------------------------------------------------------
select cant-cant_ into find2 from stock_mat where mat_cod=mat_ and dep_cod=dep and suc_cod=suc;--magiaa, cuenta si al restar queda menos de cero
select count(1) into find from detalle_ajuste_m where mat_cod=mat_ and aju_cod=cod and dep_cod=dep and suc_cod=suc and mot_cod=mot and tmot_cod!=tmot;
...
insert into detalle_ajuste_m values(cod,mat_,dep,suc,mot,tmot,cant_);--select * from detalle_ajuste_m
update stock_mat set cant=cant-cant_ where suc_cod=suc and dep_cod=dep and mat_cod=mat_;
	--raise notice '%','NOPE';
       -- return 0;
--end if;
...
raise notice '%','Se designó el Codigo '||cod||' para evitar conflictos';
      ...
      INSERT INTO remision VALUES (cod,suc,'VIG',emp,current_date);
      raise notice '%','NOPE';
      return 0;

      WHEN 2 THEN UPDATE remision SET est_cod='PRV' WHERE rem_cod=cod;
      raise notice '%','NOPE';
      return 0;

      WHEN 3 THEN UPDATE remision SET est_cod='ANU' WHERE rem_cod=cod;
      raise notice '%','NOPE';
      return 0;
end case;
    end;
