...
if (isset($_REQUEST['accion']) and $_REQUEST['accion'] == "insert") {
    ...
    try {
        $stmt2 = $dbconn->prepare("SELECT * FROM tipo_vehiculo WHERE nom_tipvehiculo = :nom_tipvehiculo");
        $stmt2->bindValue(":nom_tipvehiculo", $nom_tipvehiculo);
        $stmt2->execute();
        $cliente = $stmt2->fetch(PDO::FETCH_ASSOC);
        if (!empty($cliente['nom_tipvehiculo'])) {
            $message = "Ya se encuentra registrado un tipo de vehiculo con esa descripcion = " . $nom_tipvehiculo;
            $status = "error";
